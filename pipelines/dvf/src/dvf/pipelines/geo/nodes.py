...
Hors-Spark (Spark n'a ni connecteur PostGIS ni support géométrie). On télécharge
les contours pré-simplifiés Etalab 2025 (3 niveaux de détail) et on les charge
dans des tables référentiel PostGIS, une colonne géométrie par niveau (LOD).

Ces contours ne dépendent ni du type de bien ni des prix : ils sont chargés une
seule fois et joints aux agrégats par l'API au moment de la requête.
"""
...
# DDL des tables référentiel. Géométries en MultiPolygon/4326 (certaines communes
# et départements sont multi-parties : on uniformise via ST_Multi).
_DDL = {
    "commune_geometry": """
        CREATE TABLE IF NOT EXISTS commune_geometry (
            code_commune     text PRIMARY KEY,
            nom_commune      text,
            code_departement text,
            geom_low         geometry(MultiPolygon, 4326),
            geom_mid         geometry(MultiPolygon, 4326),
            geom_high        geometry(MultiPolygon, 4326)
        );
    """,
    "departement_geometry": """
        CREATE TABLE IF NOT EXISTS departement_geometry (
...
def _load_table(
    ...
    """Crée la table, charge les 3 LOD, pose les index. Renvoie le nb d'entités.
    ...
    with conn.cursor() as cur:
        ...
        for geom_col_name in LOD_COLUMNS.values():
            cur.execute(
                sql.SQL(
                    "CREATE INDEX IF NOT EXISTS {idx} ON {tbl} USING gist ({geom});"
                ).format(
                    idx=sql.Identifier(f"idx_{table}_{geom_col_name}"),
                    tbl=tbl,
        ...
        if "code_departement" in columns:
            cur.execute(
                sql.SQL(
                    "CREATE INDEX IF NOT EXISTS {idx} ON {tbl} (code_departement);"
                ).format(idx=sql.Identifier(f"idx_{table}_dept"), tbl=tbl)
            )
