...
def ensure_raw_csv(source_url: str) -> str:
    ...
    précédent), aucun téléchargement. Permet d'exécuter le pipeline depuis un
    clone vierge sans déposer manuellement le fichier (~94 Mo). Le téléchargement
    passe par un fichier ``.part`` puis un rename atomique : un download interrompu
    ...
    try:
        with httpx.stream(
            "GET", source_url, follow_redirects=True, timeout=_DOWNLOAD_TIMEOUT
        ) as resp:
...
def to_bronze(
    geo_dvf_raw: DataFrame, raw_csv_ready: str, repartitions: int
) -> DataFrame:
    ...
    lecture du catalog). Le DataFrame est chargé avec le schéma explicite (catalog) ;
