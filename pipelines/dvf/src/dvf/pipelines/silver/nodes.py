...
Règles méthodologiques (validées au profilage Phase 1) :
...
- Bornage des aberrations PAR département × type_local (un seuil national
  écraserait Paris).
"""
...
def filter_quality(
    bronze: DataFrame, natures_retenues: list[str]
) -> DataFrame:
...
def deduplicate_mutations(
    clean: DataFrame, types_batis: list[str]
) -> DataFrame:
    """Réduit à une ligne par mutation, caractérisée.

    ``valeur_fonciere`` étant constante par mutation, on la récupère via
    ``first`` (jamais ``sum``). La surface bâtie totale n'agrège que les locaux
    bâtis (Maison/Appartement) ; les dépendances/terrains en sont exclus.
    """
...
def compute_price_per_m2(
    ...
    y est non ambiguë. Le bornage p_low/p_high est calculé indépendamment pour
    chaque (département, type_local).
    """
    biens = (
        mutations.filter(
